#!/usr/bin/env python3
"""
Windows-Compatible LangChain Security Agent
...
def create_langchain_agent_file():
    """Create LangChain agent file without Unicode issues."""
...
class DroneSecurityAgent:
    ...
    def _initialize_openai_agent(self):
        ...
        try:
            ...
            print("WARNING: LangChain not installed. Using local agent.")
            self._initialize_local_agent()
        except Exception as e:
            print(f"WARNING: OpenAI agent initialization failed: {e}")
            self._initialize_local_agent()
...
'''
    ...
    try:
        ...
        with open("src/analysis/langchain_agent.py", "w", encoding='utf-8') as f:
            f.write(langchain_content)
        print("SUCCESS: LangChain agent file created!")
        return True
    except Exception as e:
...
def test_import():
    ...
    try:
        ...
        print(f"SUCCESS: Analysis completed! Risk: {result['risk_level']}")
...
def main():
    """Main function."""
    print("LANGCHAIN AGENT CREATOR (Windows Compatible)")
    print("=" * 50)
    
    if create_langchain_agent_file():
        if test_import():
            print("\\nALL DONE! LangChain agent is ready!")
            print("Run: python test_langchain.py")
        else:
            print("\\nFile created but import failed. Check the error above.")
    else:
